...
* add minion to Gemfile

    gem 'minion', :git => "http://github.com/Paxa/minion.git"

* Write script/minion like this
    ...
    Minion.error do |exception, queue, message, headers|
    	warn "got an error processing queue #{queue}"
...
* Run on deploy like 
    ...
    `cd #{latest_release} && RAILS_ENV=production ruby script/minion -d 5`
